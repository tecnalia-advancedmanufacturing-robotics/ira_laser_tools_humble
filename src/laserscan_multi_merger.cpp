...
LaserscanMerger::LaserscanMerger() : Node("laserscan_multi_merger")
{
	...
	param_callback_handle_ =
		this->add_on_set_parameters_callback(std::bind(&LaserscanMerger::reconfigureCallback, this, _1));
	...
	point_cloud_publisher_ =
		this->create_publisher<sensor_msgs::msg::PointCloud2>(cloud_destination_topic.c_str(), rclcpp::SensorDataQoS());
	laser_scan_publisher_ =
		this->create_publisher<sensor_msgs::msg::LaserScan>(scan_destination_topic.c_str(), rclcpp::SensorDataQoS());
}

rcl_interfaces::msg::SetParametersResult
LaserscanMerger::reconfigureCallback(const std::vector<rclcpp::Parameter> &parameters)
{
...
void LaserscanMerger::scanCallback(sensor_msgs::msg::LaserScan::SharedPtr scan, std::string topic)
{
	...
	try
	{
		...
		tf_buffer_->lookupTransform(scan->header.frame_id.c_str(), destination_frame.c_str(), scan->header.stamp,
									rclcpp::Duration(1, 0));
	...
	if (totalClouds == clouds_modified.size())
	{
		...
		// Publish point cloud after publishing laser scan as for some reason moveFromPCL is causing
		// getPointCloudAsEigen to throw a segmentation fault crash
...
void LaserscanMerger::pointcloud_to_laserscan(Eigen::MatrixXf points, pcl::PCLPointCloud2 *merged_cloud)
{
	...
	pcl::PointCloud<pcl::PointXYZI>::Ptr cloud_with_intensities(new pcl::PointCloud<pcl::PointXYZI>);
	pcl::fromPCLPointCloud2(*merged_cloud, *cloud_with_intensities);

	uint32_t ranges_size = std::ceil((output.angle_max - output.angle_min) / output.angle_increment) + 1;
	RCLCPP_DEBUG(this->get_logger(), "ranges_size: %d\n", ranges_size);
	...
	for (int i = 0; i < points.cols(); i++)
	{
		...
		if (output.ranges[index] * output.ranges[index] > range_sq)
		{
			...
			output.intensities[index] = double(cloud_with_intensities->points[i].intensity);
		}
	}
